 ...
 */
if ( ! class_exists( 'myCRED_Anspress_Get_Vote_Down_Hook' ) ) :
	class myCRED_Anspress_Get_Vote_Down_Hook extends myCRED_Hook {
         ...
         *
         * Award or deduct points of the user who gets a vote down
		 *
		 * @param $post_id
		 *
		 * @since 1.0.0
		...
		public function anspress_vote_down( $post_id ) {
			...
			if( $this->has_entry( 'anspress_get_vote_down', 'anspress_get_vote_down',$author_id, $this->prefs['creds'], $this->prefs['log'] ) ) return;
            if ( $this->over_hook_limit( 'limit', 'anspress_get_vote_down', $author_id, '' ) ) return;

			$resp = $this->core->add_creds(
				'anspress_get_vote_down',
                $author_id,
				$this->prefs['creds'],
				$this->prefs['log']
			);
