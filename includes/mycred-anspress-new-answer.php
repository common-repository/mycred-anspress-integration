 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'myCRED_Anspress_New_Answer_Hook' ) ) :
	class myCRED_Anspress_New_Answer_Hook extends myCRED_Hook {
		...
		function __construct( $hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY ) {
			...
			parent::__construct( array(
				'id'       => 'anspress_after_new_answer',
				'defaults' => array(
					'creds'    => 10,
					'log'      => '%plural% for a new answer in Anspress.',
					'limit'    => '1',
					'limit_by' => 'day'
				)
			), $hook_prefs, $type );
		...
		public function run() {
			...
			add_action( 'ap_after_new_answer', array( $this, 'anspress_new_answer' ), 10, 2 );

		}

		/**
		 * Calls When a user answers anspress question
		 *
		 * @param $post_id
		 * @param $post
		 *
		 * @since 1.0.0
		 */
		public function anspress_new_answer( $post_id, $post ) {
			...
			if( $this->has_entry( 'anspress_after_new_answer', $post_id,$user_id, $this->prefs['creds'], $this->prefs['log'] ) ) return;
            if ( $this->over_hook_limit( 'limit', 'anspress_after_new_answer', $user_id, '' ) ) return;

			$resp = $this->core->add_creds(
				'anspress_after_new_answer',
				$user_id,
				$this->prefs['creds'],
				$this->prefs['log']
			);
