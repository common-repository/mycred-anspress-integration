 ...
 */
if ( ! class_exists( 'myCRED_Anspress_Vote_Up_Hook' ) ) :
	class myCRED_Anspress_Vote_Up_Hook extends myCRED_Hook {
		...
		function __construct( $hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY ) {
			...
			parent::__construct( array(
				...
				'defaults' => array(
					'creds'    => 10,
					'log'      => '%plural% for vote up in Anspress.',
					'limit'    => '1',
					'limit_by' => 'day'
				)
			), $hook_prefs, $type );
		...
		public function run() {
			...
			add_action( 'ap_vote_up', array( $this, 'anspress_vote_up' ) );
		 ...
		 * @since 1.0.0
		 */
		public function anspress_vote_up( $post_id ) {
			...
			if( $this->has_entry( 'anspress_vote_up', 'anspress_vote_up',$user_id, $this->prefs['creds'], $this->prefs['log'] ) ) return;
            if ( $this->over_hook_limit( 'limit', 'anspress_vote_up', $user_id, '' ) ) return;
